<div class="card-body">
    <form id="form-modalAction" class="form"
        action="{{ $workIns->id ? route('admin.work-instructions.update', ['product' => $product->id, 'work_instruction' => $workIns->id]) : route('admin.work-instructions.store', $product->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @if ($workIns->id)
            @method('PUT')
        @endif
