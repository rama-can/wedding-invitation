<div class="card-body">
    <form id="form-modalAction" class="form"
        action="{{ $usageLogbook->id ? route('usage-logbooks.update', ['product' => $product->id, 'usage_logbook' => $usageLogbook->id]) : route('usage-logbooks.store', $product->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @if ($usageLogbook->id)
            @method('PUT')
        @endif
